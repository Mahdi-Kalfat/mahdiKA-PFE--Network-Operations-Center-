...
import "@utcp/file";
import { ensureCorePluginsInitialized, UtcpClientConfigSerializer } from "@utcp/sdk";
...
async function invokeUtcpTool(toolName, input = {}) {
...
Remember: The power of this system comes from combining multiple tools in sophisticated TypeScript code execution workflows.`;
    ...
    mcp.registerTool("search_tools", {
        ...
        try {
            ...
            const routerManual = await new Promise((resolve, reject) => {
            ...
            const matchedTools = routerManual.tools.filter((tool) => {
                const toolName = tool.name.toLowerCase();
                const toolDesc = (tool.description || '').toLowerCase();
                const matches = searchKeywords.some((keyword) => toolName.includes(keyword) || toolDesc.includes(keyword));
                if (matches) {
            ...
            const toolsWithInterfaces = matchedTools.map((t) => ({
                name: `router_mcp.${t.name}`,
                description: t.description
            }));
            return {
                content: [{ type: "text", text: JSON.stringify({ tools: toolsWithInterfaces }) }]
            };
        }
        catch (e) {
...
async function initializeUtcpClient() {
    ...
    if (!selfManualDisabled && !rawConfig.manual_call_templates.some((manual) => manual?.name === selfManualName)) {
    ...
    console.log(`UTCP manual templates final: ${JSON.stringify(rawConfig.manual_call_templates.map((m) => m?.name))}`);
    const clientConfig = new UtcpClientConfigSerializer().validateDict(rawConfig);
    console.log(`Using UTCP config at ${configPath}`);
    console.log(`Configured manuals: ${JSON.stringify(rawConfig.manual_call_templates.map((m) => m?.name))}`);
    const newClient = await CodeModeUtcpClient.create(scriptDir, clientConfig);
    const loadedTools = await newClient.getTools();
    console.log(`Loaded tools from UTCP repository: ${loadedTools.map((t) => t.name).join(', ')}`);

...
CATEGORY_CYPHER = """coalesce(p.category,
    CASE
        WHEN toLower(p.label) CONTAINS 'ssid' OR toLower(p.path) CONTAINS 'ssid'
          OR toLower(p.path) CONTAINS 'wifi'                                        THEN 'wifi_info'
        WHEN toLower(p.label) CONTAINS 'ppp' OR toLower(p.path) CONTAINS 'ppp'
          OR toLower(p.path) CONTAINS 'pppoe'                                       THEN 'ppp_info'
        WHEN toLower(p.label) CONTAINS 'sip' OR toLower(p.label) CONTAINS 'voip'
          OR toLower(p.path) CONTAINS 'voip'                                        THEN 'voice_info'
        WHEN toLower(p.label) CONTAINS 'lan' OR toLower(p.path) CONTAINS 'lan'
          OR toLower(p.label) CONTAINS 'host'                                     THEN 'lan'
        WHEN toLower(p.label) CONTAINS 'rx' OR toLower(p.label) CONTAINS 'tx'
          OR toLower(p.label) CONTAINS 'power' OR toLower(p.label) CONTAINS 'temperature'
          OR toLower(p.label) CONTAINS 'bias'  OR toLower(p.label) CONTAINS 'signal' THEN 'diagnostic'
        ELSE 'basic_info'
    END
) AS category"""
...
@app.get("/tools/resolve_path")
def api_resolve_path(model: str = Query(..., description="Router model / id"),
                     role: str = Query(..., description="Canonical role, e.g. vlan_id")):
    """Resolve a canonical role to the real TR-069 path for a given router model.
    Backed by the `role` stamped on HAS_PARAM by migrate_roles.py. This is what
    makes the knowledge graph load-bearing: RaDuce calls it to know which
    vendor-specific path to write."""
    print(f"[server] resolve_path: model={model!r} role={role!r}")
    try:
        rows = db("""
            MATCH (r:Router)-[rel:HAS_PARAM {role: $role}]->(p:Parameter)
            ...
            RETURN p.path AS path, p.editable AS editable, p.label AS label
            LIMIT 1
        """, {"model": model, "role": role})
        if not rows:
            return {"model": model, "role": role, "path": None,
                    "error": "No path stamped for this (model, role)."}
        row = rows[0]
        return {"model": model, "role": role, "path": row["path"],
                "editable": row.get("editable"), "label": row.get("label")}
    except Exception as e:
        return {"model": model, "role": role, "path": None, "error": f"Server error: {e}"}
...
def tool_search_parameter(keyword: str) -> dict:
    ...
    rows = db("""
        ...
        WHERE toLower(coalesce(p.path, ""))    CONTAINS toLower($keyword)
           OR toLower(coalesce(p.label, "")) CONTAINS toLower($keyword)
        RETURN
            r.id     AS router_id,
            r.product_class AS product_class,
            r.sheet_name    AS sheet_name,
